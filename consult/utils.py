...
class Classifier:
    ...
    # Constructor
    # ------------------------------------------------------------------------------------------------------------------------------------#
    def __init__(self, clusters, classificationModel, listOfmodels, clustersMaximalSpecs, clustersMinimalSpecs,
                 defaultUserInput, classificationUpperThreshold, classificationLowerThreshold, affiliationMedianUses,
                 usesDescriptions):
        ...
        self.usesDescriptions = usesDescriptions
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------#
    # Return Uses to Present According to User Affiliations
    # ------------------------------------------------------------------------------------------------------------------------------------#
    def getUsesToPresentAccordingToAffiliationInput(self, numberOfUses):
        minMaxValue = sorted(self.userApplicationInput.ix[0], reverse=True)[numberOfUses]

        test = self.userApplicationInput.ix[0, self.userApplicationInput.ix[0] >= minMaxValue]
        test = sorted(random.sample(list(test.index), numberOfUses))
        return (self.usesDescriptions[self.usesDescriptions['id'].isin(test)])
